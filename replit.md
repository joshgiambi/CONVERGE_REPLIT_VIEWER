...
## Recent Changes (August 1-29, 2025)
...
- **Fixed CT Loading Error**: Resolved "getSlicePositionUsingIOP is not defined" error preventing CT scans from loading. Replaced non-existent function calls with existing getSliceZ function that properly handles multiple fallback scenarios for determining slice positions
- **Fixed Smooth Button**: Implemented missing smooth action handler in working-viewer.tsx to properly apply gaussian smoothing to RT structure contours
- **Fixed Boolean Operations**: Corrected boolean operation handlers to properly work with arrays of contours returned from ClipperLib operations, supporting both union (combine) and subtraction operations
- **Fixed Boolean Toolbar Text Cursor**: Resolved text cursor positioning issue in boolean operations toolbar where cursor appeared behind active regions, improving user input experience
